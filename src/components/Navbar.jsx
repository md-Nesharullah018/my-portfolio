  ...
  return (
    <div className="fixed top-0 left-0 w-full z-[100] px-4 py-4 md:px-10">
      ...
      <motion.nav
        ...
        className={`mx-auto max-w-6xl transition-all duration-500 rounded-[24px] border ${
          scrolled
            ? "bg-black/70 backdrop-blur-2xl border-cyan-500/10 shadow-[0_10px_40px_rgba(0,0,0,0.7)] py-3 px-6"
            : "bg-transparent border-transparent py-5 px-4"
        } flex items-center justify-between`}
      >
        ...
        <Link to="/" className="relative flex items-center gap-2 group">
          ...
          <div className="w-8 h-8 bg-gradient-to-br from-cyan-400 to-sky-600 rounded-lg rotate-45 group-hover:rotate-[225deg] transition-all duration-700 flex items-center justify-center shadow-[0_0_15px_rgba(6,182,212,0.25)]">

            <span className="text-white font-black -rotate-45 group-hover:-rotate-[225deg] transition-all duration-700 text-xs">
              MD
            </span>
        ...
        {/* Desktop Navigation */}
        <div className="hidden md:flex items-center gap-2 bg-white/[0.03] p-1.5 rounded-full border border-white/[0.06]">
            ...
            <Link key={link.name} to={link.path}>
              ...
              >
                ...
                {location.pathname === link.path && (
                  ...
                  <motion.div
                    layoutId="nav-glow"
                    className="absolute inset-0 bg-cyan-500/[0.08] rounded-full border border-cyan-400/20 shadow-[0_0_15px_rgba(6,182,212,0.12)]"
                  />
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={() => setIsOpen(!isOpen)}
            className="md:hidden w-11 h-11 flex items-center justify-center rounded-full bg-white/[0.03] border border-cyan-500/10 text-white text-2xl"
          >
            {isOpen ? <VscClose /> : <HiMenuAlt4 />}
          </button>
      ...
      <AnimatePresence>
        ...
        {isOpen && (
          ...
          <motion.div
            ...
            animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: 100 }}
            className="fixed inset-0 bg-[#000000] z-[99] md:hidden flex flex-col justify-center items-center gap-8"
          >

            {/* Very subtle cyan accent */}
            <div className="absolute top-[-10%] right-[-10%] w-[250px] h-[250px] bg-cyan-500/[0.04] blur-[100px] rounded-full" />
            ...
            {navLinks.map((link, i) => (
              ...
              >
                ...
                <Link
                  to={link.path}
                  onClick={() => setIsOpen(false)}
                  className="text-4xl font-black text-white hover:text-cyan-400 transition-colors uppercase tracking-tighter"
                >
                  {link.name}
                </Link>
            ...
            <motion.a
              ...
              transition={{ delay: 0.5 }}
              href="/Md_Neshar 25-06-2026.pdf"
              className="mt-10 px-10 py-4 bg-cyan-500 hover:bg-cyan-400 text-black rounded-2xl font-black uppercase tracking-widest transition-colors"
            >
              Download CV
            </motion.a>
